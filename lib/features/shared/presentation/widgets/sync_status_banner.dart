...
/// Items that stopped retrying get their own row: they are neither pending nor gone.
class SyncStatusBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final failedCount = syncStatusNotifier.failedCount;

        final rows = <Widget>[
          // If online and nothing is pending/syncing, the status row stays hidden
          if (!(isOnline && !isSyncing && pendingCount == 0))
            _statusRow(isOnline: isOnline, isSyncing: isSyncing, pendingCount: pendingCount),
          if (failedCount > 0) _failedRow(context, failedCount),
        ];
  ...
  Widget _statusRow({required bool isOnline, required bool isSyncing, required int pendingCount}) {
    Color bgColor;
    Color textColor;
    IconData iconData;
    String message;
    ...
    if (isSyncing) {
      ...
      actionWidget = _action('Sync Now', AppColors.primary, () => syncStatusNotifier.syncNow());
    }

    return _strip(bgColor: bgColor, textColor: textColor, iconData: iconData, message: message, action: actionWidget);
  }

  Widget _failedRow(BuildContext context, int failedCount) {
    const textColor = AppColors.alertCritical;
    return _strip(
      bgColor: AppColors.alertCritical.withValues(alpha: 0.10),
      textColor: textColor,
      iconData: Icons.sync_problem_rounded,
      message: '$failedCount item${failedCount == 1 ? "" : "s"} failed to sync',
      action: _action('Review', textColor, () => context.push('/sync-issues')),
    );
  }
  ...
  Widget _action(String label, Color color, VoidCallback onPressed) {
    return TextButton(
      ...
      child: Text(
        label,
        style: AppTypography.captionMetadata.copyWith(color: color, fontWeight: FontWeight.bold),
      ),
    );
  }
